...
public class CopeVoider extends Module {
    ...
    private final Setting<Integer> chunkRadius = sgGeneral.add(new IntSetting.Builder()
        .name("chunk-radius")
        .description("How many chunks in each direction to void (1 chunk = 16 blocks).")
        .defaultValue(2)
        .min(1)
        .max(20)
        .sliderMax(20)
    ...
    private final Setting<Integer> minHeight = sgGeneral.add(new IntSetting.Builder()
        .name("min-height")
        .description("Minimum height to void blocks from.")
        .defaultValue(-63)
        .min(-64)
        .max(319)
        .sliderRange(-64, 319)
    ...
    private final Setting<Integer> maxHeight = sgGeneral.add(new IntSetting.Builder()
        .name("max-height")
        .description("Maximum height to void blocks to.")
        .defaultValue(319)
        .min(-64)
        .max(319)
        .sliderRange(-64, 319)
    ...
    public CopeVoider() {
        super(Cope.CATEGORY, "copevoider", "Voids chunks between specified heights (requires op)");
    }
    ...
    private void onTick(TickEvent.Post event) {
        ...
        if (currentChunk == null) {
            ...
            if (currentChunk == null) {
                info("§aFinished voiding all chunks!");
                toggle();
                return;
            }
            currentHeight = minHeight.get();
    ...
    private void processChunkSection() {
        ...
        String fillCommand = String.format("fill %d %d %d %d %d %d minecraft:air replace",
