...
public class CopeStrike extends Module {
    ...
    private final Setting<String> nom = sgGeneral.add(new StringSetting.Builder()
        ...
        .defaultValue(" Owned by discord.gg/HQyNhNKwMv | #<ticketid> | Get better next time | §k111 §r ")
        .build());
    ...
    public CopeStrike() {
        super(Cope.CATEGORY, "Cope-Strike", "Rains things down from the sky");
    }
    ...
    public void onTick(TickEvent.Post event) {
        if (randomPrefix.get()) {
            ...
            customName = randomPrefix + replacePlaceholders(nom.get());  // Add placeholder replacement here
        } else {
            customName = replacePlaceholders(nom.get());  // Add placeholder replacement here
        }
        if (mixer.get()) {
            mix++;
    ...
    private NbtComponent createEntityData() {
        ...
        entityTag.putString("CustomName", "{\"text\":\"" + replacePlaceholders(customName) + "\",\"color\":\"" + namecolour + "\"}");
    ...
    private void executeCommandsToCreateEntities() {
        ...
        command += " {";
        command += "\"CustomName\":\"{\\\"text\\\":\\\"" + replacePlaceholders(customName) + "\\\",\\\"color\\\":\\\"" + nameColor + "\\\"}\",";
